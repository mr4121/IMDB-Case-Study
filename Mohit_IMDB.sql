USE imdb;
/* NOTE: I have used NT as prefix for names in the WITH syntaxes to mean 'New Table', 
   which makes things more clear for me */
...
SHOW tables;
-- There are 6 tables:: director_mapping, genre, movie, names, ratings, role_mapping
-- Segment 1:
...
/* Count(*) counts all the rowns including the rows with null values
so this is more suited here */
...
-- The first part, i.e., The Total Number Of Movies Released Each Year
SELECT
Year, COUNT(id) AS number_of_movies
FROM movie
GROUP BY year;

-- The second part, i.e., Month-Wise Trend
...
SELECT COUNT(DISTINCT id) AS 'Movies Produced in India or USA in 2019' 
FROM movie WHERE country IN ('India', 'USA') AND year = 2019;

-- We used 'DISTINCT' here as there are chances of a movie being produced in both the countries
...
SELECT COUNT(DISTINCT genre) as 'No. of Unique Genres' FROM genre;
...
-- Used inner join as we only needed matching data from both the tables
...
SELECT COUNT(*) AS 'Movies With Only One Genre'
...
SELECT * FROM nt_avg_rating WHERE Movie_Rank<=10;

/* We could have used limit but it would have given us only first 10 values
and we would have missed out on last few movies which share the 10th rank */
...
SELECT * FROM NT_Prod_Company_Rank
...
SELECT COUNT(id) as No_of_Movie_Released, r.Median_Rating
FROM movie m
INNER JOIN ratings r
ON m.id = r.movie_id
...
/* As we can see, Votes_Count for Germany > Votes_Count for Italy so yes
...
SELECT * FROM names;  -- to check for column names
...
top3_genre
using (genre)
WHERE r.avg_rating > 8 AND g.genre IN (top3_genre.genre)
GROUP BY n.name
ORDER BY movie_count DESC
)
...
WITH NT_date_summary AS
(SELECT d.name_id, NAME, d.movie_id,
duration, r.avg_rating, r.total_votes, m.date_published,
